...
const PracticeCard = ({ title, description, icon: Icon, iconColor, badge, onClick, disabled }: PracticeCardProps) => (
  <button 
    onClick={onClick}
    disabled={disabled}
    className="w-full bg-[#1a232e] border-2 border-white/10 rounded-[20px] flex items-stretch overflow-hidden hover:bg-[#252f3d] transition-all group active:translate-y-[2px] relative shadow-[0_4px_0_0_rgba(255,255,255,0.05)] active:shadow-none disabled:opacity-50 disabled:cursor-not-allowed"
  >
    ...
    <div className="w-20 md:w-24 flex items-center justify-center pr-5 md:pr-6">
      <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl border-2 border-white/20 flex items-center justify-center shadow-[0_4px_0_0_rgba(255,255,255,0.1)] ${iconColor} active:translate-y-[2px] active:shadow-none transition-all relative`}>
        ...
        {isLoading ? (
          ...
          <div className="relative overflow-hidden rounded-[24px] bg-gradient-to-br from-duo-green to-duo-dark-green p-6 md:p-8 min-h-[200px] flex flex-col justify-between group">
            <div className="relative z-10 w-full md:max-w-[65%] text-center md:text-left">
              <h3 className="text-2xl font-black text-white mb-2 leading-tight">Review Practice</h3>
              ...
              <button 
                ...
                disabled={!completedLessons || completedLessons.length === 0 || isLoading}
                className="bg-white text-duo-green font-black text-sm uppercase tracking-widest px-8 py-3 rounded-xl shadow-[0_4px_0_0_#e5e5e5] hover:bg-gray-50 active:translate-y-[2px] active:shadow-none transition-all disabled:opacity-50 disabled:cursor-not-allowed w-full md:w-auto"
              >
                START PRACTICE
              </button>
            </div>
